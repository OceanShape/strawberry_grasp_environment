...
- [execution/scan_executor_node.py:_ALL_CELLS_CLOCKWISE_ORDER, _compute_scan_order · execution/cell_traversal.py 모듈 docstring] **이름·주석만 변경, 동작 불변 (2026-09-17)**: 상수 `_ALL_CELLS_ZORDER` → `_ALL_CELLS_CLOCKWISE_ORDER`. 원본 값(sw→nw_flat→ne→se)도 현재 값(nw→ne→se→sw)도 시계 방향이고 Z-order(nw→ne→sw→se)가 아니라 이름이 틀렸다. 상수 위 주석에 순서 기준을 적음 — 원 팀 실기 기록(민1 STEP 6·민2 §7) 순서 그대로, 기록에 이유 없음, 인접 이동뿐인 것은 관찰일 뿐, 깊이 2 는 다른 기준(아래쪽 먼저 sw→se→nw→ne). `_compute_scan_order` 09-09 주석과 `cell_traversal.py` docstring 의 "NW 가 빠지고 3분면만 돌았다"를 "원본 순서목록을 리포 YAML 과 함께 돌리면 빠질 수 있던 조합 결함, 실기 동작 아님(4셀 순회 실기 검증, 3분면 런 로그 0건)"으로 재작성. 따라 바꾼 곳: `check_params.py` 검사 정규식, `scripts/compute_subcell_scan_poses.py` 주석, 문서(G §5·§6-1·§8·§9, H·F·PLANNER_POLICY_v2·docs/parameters.md 이름). colcon build 반영
