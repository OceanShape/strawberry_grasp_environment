...
- [assets/props/egg_carton.usd, scenes/main_scene.usd, scenes/layers/layout_layer.usd] **T4-3 계란판 시각 메쉬** — 5×3 컵 2,184점/983면, 물리·콜라이더·조인트 없음, 이름에 `strawberry` 없음(§2 게이트). **좌표 신규 생성 없음**: 격자 벡터는 플래너 상수 `TAUGHT_SLOT{0,1,3}_PLACE_REFERENCE` 그대로(84.26° 평행사변형 유지), 원점은 런 7 과실 정지 위치 6개의 최소자승. 격자 상수는 ee 위치라 그 자리에 놓으면 250mm 어긋난다는 함정을 fit 으로 우회. **점유 컵 6개는 실측 과실 위치에 정렬**(격자 대비 2.9~6.0mm — 분면별 접근 기울기로 매달린 깊이 8.7mm 차, 행 피치 51.2 < 과실 53.8 이라 림으로 흡수 불가). 컵 벽은 **과실 반경 프로파일 + 3mm**(4링), 바닥은 상판 위 +1mm 하한(격자 z 기울기로 바닥이 전부 상판 아래로 가 빈 컵에 테이블이 비치는 것을 막음), 스커트는 상판 5mm 아래로 앉힘. 시행착오: 바닥 −36mm 고정 → 상판 아래 / 바닥만 올림 → 벽이 과실 파고듦 5.2mm / 프로파일 벽 + 정렬 → 관통 0.0mm (2026-09-11)
- [scripts/scene_tools/egg_carton_geom.py, gen_egg_carton_asset.py, verify_egg_carton.py] 신설 — 형상 단일 출처 / 생성기 / 검증기(일반 python3 + usd-core). 검증: 물리 0건·발행 필터·점유 컵 정렬·과실 6개 컵 벽 관통 0.0mm·바닥 15개 상판 위·스커트 = 쉘 최저점. 배치 슬롯이나 과실 정지 위치가 바뀌면 `FRUIT_REST_M` 갱신 후 재생성·재검증 (2026-09-11)
