...
- [assets/props/egg_carton.usd, scenes/main_scene.usd, scenes/layers/layout_layer.usd] **T4-3 계란판 시각 메쉬 (2차)** — 수평·직사각·컵 15개 동일, 2,188점/984면, 물리·콜라이더 없음, 이름에 `strawberry` 없음. 피치만 플래너 상수 `TAUGHT_SLOT{0,1,3}` 의 크기(59.8×51.2mm). 배치는 티칭 ee 격자 + 평균 ee→과실 변위(런 7)에 강체 최소자승 → translate (0.7676, 0.0879, 0), yaw −7.24°, 컵 중심 잔차 ≤5.3mm. **1차(같은 날) 폐기**: 티칭 격자(84.26°, z 기울기)를 판에 그대로 굽고 점유 컵을 과실에 맞춘 것 — 사용자 지적대로 "균일한 컨테이너" 라는 계란판의 존재 이유를 없앴다. 격자 왜곡은 티칭 오차이지 컨테이너 성질이 아니므로 컨테이너는 규칙적으로 두고 **과실이 컵에서 벗어나는 모습**(런 7: 치우침 ≤5.8mm, 림/벽 겹침 ≤7.6mm)으로 드러낸다. 각도 대안은 `YAW_OVERRIDE_DEG`(0° → 잔차 16.7mm) (2026-09-11)
- [scripts/scene_tools/egg_carton_geom.py, gen_egg_carton_asset.py, verify_egg_carton.py] 신설 — 형상 단일 출처 / 생성기 / 검증기(일반 python3 + usd-core). 검증: 물리 0건·발행 필터·배치=fit·판 수평·직교 격자·컵 15개 동일·과실 6개 자기 셀 안(치우침·겹침은 정보). 배치 슬롯이나 과실 정지 위치가 바뀌면 `FRUIT_REST_M` 갱신 후 재생성·재검증 (2026-09-11)
