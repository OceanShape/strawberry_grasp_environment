...
**아직 고치지 않았다.** 타이밍이 시퀀스 전체에 걸리는 변경이라 별도 런으로 검증해야 하고, 지금 배치는
이미 안 닿는다(+12.7mm). T5 녹화를 먼저 할지, 이 값을 조이고 한 번 더 돌릴지는 판단 대상이다.
조인다면 `arm_arrival_tol_deg` ROS 파라미터 + **정체 감지**(개선이 멈추면 대기 종료)를 같이 넣어
8초 타임아웃이 새로 생기지 않게 한다.
