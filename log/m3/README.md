...
| **런 7** (대기) | — | **도착 허용오차 1.5 → 0.3deg** + 정체 감지 + 잔차 계측 (`ARM_ARRIVAL`) |
...
# 런 7 — (대기) 도착 허용오차 1.5 → 0.3deg

`ARM_ARRIVAL_TOL_DEG` 1.5 → **0.3** (`arm_arrival_tol_deg` ROS 파라미터로 덮어쓸 수 있다).
배치 자세 기준 손끝 예산 **RMS 23.6mm → 4.7mm**. 같은 게이트가 파지 접근에도 걸리므로 파지 정밀도도 같이 본다.

함께 들어간 것:
- **정체 감지** — 개선이 0.01deg 미만인 상태가 0.30s 이어지면 대기를 끝낸다(`ARM_ARRIVAL_STALLED`, WARN).
  조인 값 때문에 8초 타임아웃이 새로 생겨 런이 통째로 느려지는 것을 막는다.
  단 **잔차가 2.0deg 이하일 때만** 정체로 인정한다 — 팔이 아예 안 움직이는 실패(09-09 17:23 브릿지 콜백 사망,
  잔차 130deg)를 0.45초 만에 "도착" 으로 삼키지 않게 하는 상한이다. 그 경우는 종전대로 8초 타임아웃.
- **잔차 계측** — 모든 도착에 `ARM_ARRIVAL <label>: 잔차 X.XXdeg (손끝 X.Xmm) / 대기 X.XXs` 를 남긴다.
  어디까지 수렴하는지가 보여야 값을 다시 정할 수 있다.
- **손끝오차 계산 버그 수정** — `_tip_error_mm` 신설. 종전에는 `get_state()` 를 두 번 부르고 `.copy()` 없이
  두 결과를 뺐다. cuRobo 가 내부 버퍼를 재사용하므로 **항상 0mm** 가 나오던 값이다
  (SUBMISSION_PLAN T1 "부수 발견: 손끝오차 수치 인용 금지" 가 이것). 이제 신뢰할 수 있다.

세 갈래 경로는 오프라인 단위 시험으로 확인했다 — 수렴 0.32s(info) / 0.8deg 정체 0.52s(warn) / 130deg 고정 8.0s(error, ret False).

**`colcon build --packages-select strawberry_sim_core` 완료.** 노드 재기동만 하면 된다 (씬 재로드 불필요).

## 판정표 (채울 것)

| # | 항목 | 기준 |
|---|---|---|
| 1 | 기동 대조 | `arm_arrival_tol=0.30deg` OK (run_nodes.sh 가 자동 확인) |
| 2 | **팔 도착 잔차** | `ARM_ARRIVAL` 손끝값 분포. 런 6 은 이 값이 없었고, 과실에서 역산한 팔 오차가 10.0~24.3mm 였다 |
| 3 | `ARM_ARRIVAL_STALLED` | 몇 건인지 · 그때 잔차 몇 deg 인지. 많으면 0.3 이 과하다는 뜻 → 0.5 로 완화 후보 |
| 4 | `ARM_ARRIVAL_TIMEOUT` | **0건이어야 한다.** 나오면 정체 상한(2.0deg)을 넘는 진짜 실패다 |
| 5 | 전체 시간 | 런 6 은 213.8초. 대기가 늘어 +20초 이내면 허용 |
| 6 | **배치 산포** | `RELEASE frozen at` 6개 vs 티칭 격자. 런 6 은 계통 11.0mm + 산포 11.0mm 였다 |
| 7 | **파지 정밀도** | `GRASP_JUDGE` lateral. 런 6 은 1.3~4.3mm |
| 8 | 종전 기준 | `PICK COMPLETE` 6, `_PLACE_BLOCKED`·`clamped`·`JOINT_COMMAND_REJECTED`·`EXEC_*` 0건 |

남는 (B) 티칭 격자 84.26° 는 이 변경과 무관하다 — 계통 기울기는 그대로 남는다.
