...
# 런 6 — (대기) 슬롯 `0,1,6,7,12,13` + MoveLine 측방 오차 계측

**`colcon build --packages-select strawberry_sim_core` 는 09-10 에 이미 돌려 뒀다.** 노드 재기동만 하면 된다.
씬 재로드는 T3 덩굴을 이미 본 뒤라면 불필요하다.

| # | 항목 | 기준 |
|---|---|---|
| 1 | 시퀀스 | `slot 0 -> 1`, `1 -> 6`, `6 -> 7`, `7 -> 12`, `12 -> 13` 5건 |
| 2 | slot 12·13 배치 | `_PLACE_BLOCKED` 0건. **slot 13 은 사전 검증 5/7 이라 시퀀스 맨 뒤에 뒀다** — 실패하면 그 자리에 놓고 끝난다(`hold_on_place_failure=false`) |
| 3 | 배치 간격 | 여섯 개가 안 닿는다. 열 이웃(0↔1, 6↔7, 12↔13) 이상 간격 +15.3mm |
| 4 | **`MOVELINE_END_ERR`** | 배치 하강 6건의 `lateral` 값 — (C) 의 정량치. 이 값이 (B) 5.74° 로 설명 안 되는 잔차의 크기다 |
| 5 | 종전 기준 | `PICK COMPLETE` 6, `clamped`·`ARM_ARRIVAL_TIMEOUT`·`JOINT_COMMAND_REJECTED`·`EXEC_*` 0건 |

**T4-3 계란판 컵 좌표 (티칭 격자 기준 예상, mm)** — 실제 컵은 런 6 의 `RELEASE ... frozen at` 으로 확정한다.
