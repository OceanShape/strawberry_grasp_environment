...
| 남은 것 | Isaac 화면 확인(색 반영, HUD 패널과 그리퍼 카메라 창 간격) — §8 |
...
1. **반영 순서**: 씬 재로드(새 서브레이어) → 브릿지·뷰포트 표시 스크립트 Run → `bash scripts/run_nodes.sh --kill` → `bash scripts/run_nodes.sh`(프로브는 노드가 뜰 때 읽힌다) → 트리거.
...
5. **패널과 좌하단 그리퍼 카메라 창 간격**: 스텁 레이아웃 추정으로 패널 아래 끝 약 516\~528px. 뷰포트 높이 1000px 이면 40px 이상 여유, 960px 이면 거의 붙고, 약 956px 보다 낮으면 범례 줄이 카메라 창에 가린다. 녹화 레이아웃의 실제 뷰포트 높이로 확인한다.
6. 플래너 로그의 `[harvest_probe] 결과 바 n번째 …` 줄 수 = 범례 합계.
