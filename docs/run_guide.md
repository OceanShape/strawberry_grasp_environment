...
렌즈(18.147 = hfov 60°, 31mm 환산)라 보드가 화면 폭 **24%** 였고 원근이 과장됐다. S7 값은 보드 **34%**, 트레이 x 75\~85%.
조언은 40% 였지만, 좌상단 HUD 패널(x<24%, 당시 y<43% — 09-19 기준 y<54%)과 좌하단 그리퍼 카메라 창(x<27%, y>60%)을 피해 피사체를 x≥28% 에 두면
...
단계            하강 + 파지                     단계별 색
       ■■■■□□□□□□□                              11칸 진행 바
──────────────────────────────────────────
          수확 진행 중                          (09-19) 한 줄, 같은 자리, 상시 — 런 전 `수확 시작 전`, 런 중 `수확 진행 중`(둘 다 흐린 회색),
     (또는) 수확 완료  5 / 8 (62%)              완주 후 `수확 완료 n / N`, 완료 호출 없이 끝난 런은 `수확 중단`(흰색). 괄호 = 목표 대비 %
...
- **트리**(2026-09-11, 영역·타겟·배치 줄 대체)는 `scan_executor` 의 순회 결정을 실행 중에 그린다. 1차 스캔에서 후보 0 인
  ...
  순회가 끝나면 세부 칸 줄은 사라지고 그 자리는 높이 그대로 빈칸으로 남는다(09-19, 아래 패널이 안 움직이게). 보드 위 하늘색 테두리 하이라이트(09-16, 살구색 면 채우기에서)도 세부 칸에서 일할 때는 그 칸 하나만 켠다(`whiteboard.usd` 세부 칸 16장,
...
- **패널 위치**는 `isaac_sim_viewport_display.py` 의 `POS_X` / `POS_Y` (뷰포트 좌상단 기준 픽셀, 기본 16/32).
  09-17 에 두 행이 늘고 09-19 에 결과 바 위 줄(시작 전·진행 중·중단·완료)이 런 내내 자리를 차지해, 패널 아래 끝이 런 내내 약 578px 이다(완주 뒤에도 같다) — 녹화 절차대로 뷰포트 렌더 해상도를 1920×1080 으로 맞추고 디스플레이도 1080p 로 두면(`isaac_sim_fit_viewport_1080p.py`) 좌하단 그리퍼 카메라 창 위 끝(약 648px)과 약 70px 떨어진다. 완주 때 2단 트리 자리는 비우되 높이를 남겨 패널의 줄 위치가 바뀌지 않는다(09-19).
...
- **직전 런의 잔상** (2026-09-10 수정): HUD 는 자기가 Run 된 시각보다 먼저 멈춘 스냅샷 파일을 무시하고,
  ...
  다시 띄우면 트리거 전까지 **대기 상태(램프만 순서대로 초록, 09-19 부터 결과 바 위 `수확 시작 전`)** 로 보인다. 완주 후 노드를 그대로 두면
