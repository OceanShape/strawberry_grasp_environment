...
| 부팅 때 자동 | 내용 |
|---|---|
| 뷰포트 HUD 끄기 | FPS·프레임타임·GPU/프로세스 메모리·해상도 오버레이. **녹화본에 안 찍힌다** |
...
**`taught_slot_sequence:="0,1,3,4,6,7"` — 배치 슬롯을 **인접 칸**으로 여섯 칸 진행한다. (2026-09-11 T4-3 4차)**
...
| **`root/se` 에서 `TARGET_NOT_FOUND`** | **정상이다.** 씬의 익은 딸기 6개는 sw3 / nw2 / ne1 / **se0** 으로 배치돼 있다. se 에는 익은 딸기를 두지 않았다 (병든 딸기 분기는 제출 범위 밖 — [`SUBMISSION_PLAN.md`](../SUBMISSION_PLAN.md) §5) |
