...
LOG_ROOT = Path(os.path.expanduser(
    "~/strawberry_grasp_environment/src/e0509_gripper_description/logs/gripper_calibration"))
...
class SafeGraspTrial(Node):
    def __init__(self, action_name, set_position_service):
        super().__init__("safe_grasp_trial_logger")
        self.client = ActionClient(self, SafeGrasp, action_name)
        self.set_position_client = self.create_client(SetPosition, set_position_service)
        self.feedback = []
    ...
    def pre_open(self, position, timeout_sec, server_timeout_sec):
        if not self.set_position_client.wait_for_service(timeout_sec=server_timeout_sec):
            raise RuntimeError("SetPosition service unavailable")
        ...
        if result is None or not result.success:
            ...
            raise RuntimeError(f"pre-open set_position failed: {message}")
        print(
            f"pre_open position={result.present_position} "
            f"current={result.present_current} message={result.message}")
...
def main():
    ...
    parser.add_argument(
        ...
        choices=("empty", "stem", "stem_moru", "stem_tape", "leaf_or_non_target"))
    ...
    parser.add_argument("--set-position-service", default="/gripper_service/set_position")
    parser.add_argument(
        "--pre-open-position", type=int,
        help="SafeGrasp 실행 전에 이 위치로 먼저 연다. 예: 600")
    parser.add_argument("--pre-open-timeout-sec", type=float, default=5.0)
    ...
    rclpy.init()
    node = SafeGraspTrial(args.action_name, args.set_position_service)
    try:
        if args.pre_open_position is not None:
            node.pre_open(
                args.pre_open_position,
                args.pre_open_timeout_sec,
                args.server_timeout_sec)
