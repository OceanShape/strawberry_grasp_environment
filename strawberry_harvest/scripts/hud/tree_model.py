...
# ---- 색 (0-255 RGBA) ----------------------------------------------------------------
#: 로봇이 지금 있는 노드와 경로. 보드 하이라이트와 같은 색으로 맞춰
#: 보드에서 불 켜진 칸과 트리에서 불 켜진 노드가 같은 곳으로 읽히게 한다.
#: [2026-09-16 S6] 보드가 살구색 면 채우기 → 하늘색 테두리 발광으로 바뀌면서 같이 하늘색으로.
#: 채도는 같은 날 낮췄다(#38BDF8 → #81B8DC) — PiP 가이드의 초록과 헷갈려서. 보드 색의 출처는
#: scene_tools/gen_board_highlight.py 의 GLOW_SRGB 다. 바꾸면 여기도 같이 바꾼다.
ACTIVE = (129, 184, 220)
