...
에러처럼 읽혔다. **단계 색은 이 규칙 밖이다** — `당김`(0xFF3D5C, DETACH 단계 라벨. 2026-09-18 전 이름은 '분리' — 결과 바 '분리 실패'가 진입부터 배치 호출 직전까지 구간 전체(후퇴 포함)를 가리키게 되면서 같은 글자가 한 화면에 두 뜻으로 뜨는 것을 피해 바꿨다)처럼 빨간 계열이 있지만 그건 "지금 이 단계"라는 뜻이고
