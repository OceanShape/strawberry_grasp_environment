...
- **1단** NW·NE·SE·SW (위치 순서). 숫자는 1차 스캔 후보 수였다가 분면 자세에서 센 후보 수로 바뀐다.
  ...
  손목 카메라 창의 십자·NW/NE/SW/SE 가이드가 방위를 보여 주고 첫 줄이 이미 NW 라서, 24px 로 키운 4열 패널에 넣을 이유가 없다.
...
- **색** 시안(`#38BDF8`, 09-16 까지 주황) = 로봇이 지금 있는 노드와 경로(보드 하이라이트와 같은 색), 초록 테두리 = 끝남,
  호박색 테두리 = 세부 자세 유도가 거부돼(`SUBDIVIDE_REJECTED`) 부모 자세에서 딴 세부 칸.
- **보드 하이라이트도 세부 칸까지 내려간다** (2026-09-12). 로봇이 세부 칸에서 일하면(트리의 시안 노드가 2단이면) 보드 위 그 칸
  하나만 켠다 — `whiteboard.usd` `highlight/<분면>_<세부 칸>` 16장(분면 4장을 2×2 로 나눈 것). 옛 애셋(세부 칸 없음)이면
  부모 분면을 켜고 Kit 콘솔에 한 줄 알린다. **씬 재로드 필요.**
- **보드 하이라이트 모양 (2026-09-16 S6)**: 살구색 면 채우기 → **시안 발광 테두리**. 분면 띠는 텍스처 격자선과 같은 20mm 로 그 선 위에
...
```
┌ 손목 카메라 · D455 컬러 렌더 · 인식 없음   영역 NE ┐
│                    │                     │   640×480 렌더를 480×360 으로 표시 (09-16 S5, 400×300 에서)
│                SE  │  SW                 │   뷰포트 좌하단, 여백 16px
│ ───────────────────┼─────────────────── │   초록 십자 + 방위 4개 (09-16, 아래)
│                NE  │  NW                 │   캡션 끝의 '영역' = HUD 의 현재 영역
│                    │                     │
└──────────────────────────────────────────┘
```

**십자·방위 가이드 (2026-09-16).** 실기 비전 노드 창이 그리는 것과 같은 **화면 고정 가이드**다 — 화면 중앙 십자와 교차점에 모인
`NW·NE·SW·SE`, OpenCV 순수 초록 `#00FF00`(캡처에서 연두로 보이는 건 압축 탓) + 1px 검정 외곽선(안 익은 과실 위에서도 읽히게).
글자 높이는 창 높이의 5%, 교차점에서 9px 띄운다(`CAM_GUIDE_*`). **인식 결과가 아니다** — 아래 두 가지가 이 그림을 정직하게 만든다:
...
- **방위 글자 자리는 실기 영상에서 읽은 배치 그대로다** (사용자 확인 09-16): 넷이 교차점에 모이되 각자 **건너편**에 앉는다 —
  NW 우하, NE 좌하, SW 우상, SE 좌상. `CAM_GUIDE_SLOT` 한 곳만 이 배치를 안다.

캡션 끝 `영역 NE` 는 HUD 의 현재 영역(하이라이트 키와 같은 값: `HOME`·`NE`·`SW/SE`)이다. 분면 자세에선 교차점이 화면 밖으로 나가니
