...
단계            하강 + 파지                     현재 단계 (34px, 단계별 색, 가운데 정렬)
       ■■■■□□□□□□□                              11칸 진행 바 (스캔 이동 → 완료)
──────────────────────────────────────────
          수확 완료  5 / 6 (83%)                완료 시에만 (가운데 정렬, 흰색). 괄호는 목표 대비 백분율
              배치 5   낙하 1                    (2026-09-15) 둘째 줄: 트레이 슬롯 릴리스 수(초록) · 이송 거부로 놓아 버린 수(빨강)
```

**색 규칙 (2026-09-16)**: **빨강은 실패에만** 쓴다 — 죽은 노드 램프와 `낙하 m`. 잘 된 수(`배치 n`)는 초록(`C_OK`,
배치·완료 단계와 같은 초록), 제목과 비율은 흰색이다. 09-16 전까지 `수확 완료` 줄은 분홍빨강(`C_ACCENT` 0xFF6B81)이라
...
여백을 조절한다. **글자 크기 (2026-09-16 S5, 1080p 녹화 기준)**: 본문·노드 24px, 머리 22px, 핵심 수치 `배치 n · 낙하 m` 32px,
...
**낙하 (2026-09-15, T4c)**: 분리까지 된 과실을 이송·배치 계획 거부로 트레이 밖에서 놓아 버린 수를 `result.dropped` 로 세어
둘째 줄에 `낙하 m` 으로 낸다. 세는 자리는 실기 실행기의 `_release_and_continue_after_place_failure` 경계(프로브 래핑) —
