...
무엇을 재나: 익은 과실(타겟, 현재 8개)마다 **덩굴 수직 구간 = 과실중심 +25~+65mm** 선분을 잡고,
그 선분에서 다른 과실 11개의 메시 표면까지 최소거리를 구한다. 열린 조우는 이 선분을 따라
+65mm(하강 시작)에서 +35mm(파지 목표)까지 내려오므로, 이 거리가 조우 폭보다 작으면
그리퍼 파츠가 이웃 과실과 화면상 겹친다. 과실에는 콜라이더가 있지만, 과실에 닿는 커스텀 손끝 파츠
(robot.urdf left/right_custom_part)에는 <collision> 이 없다 (H §4).
